...
fn main() {
    ...
    let result = samtools_extractor(&args.alignment_arg,
          &args.idlist_arg,
          &args.fasta_arg,
          args.upstream_arg,
          args.downstream_arg,
...
fn samtools_extractor(pathsam: &str, pathfasta: &str, pathlist:&str,
                             upstream: usize, downstream: usize, readlength: usize)
  ...
  for i in limit_lines.iter(){
    ...
    if mutable.is_empty() {
      ...
      limit.push(Limit {
        ...
        start: mutable[3].parse::<usize>().unwrap()});
    }
  }

   // making a subset to reduce the iteration and restrictive search iteration.
    ...
    for j in subset.iter() {
      if j.header == i.headerline {
        final_struct.push(LimitFinal{
          ...
          read: i.line.clone(),
          sequence: j.sequence.clone(),
          upstream: j.sequence[0..i.start-upstream].to_string(),
          downstream: j.sequence[i.start+readlength..j.sequence.len()].to_string()
        })
      }
    }
  }
  ...
  let mut upstream_fasta_write = File::create("selected-ids-upstream.fasta").expect("file not present");
  for i in final_struct.iter_mut(){
    write!(upstream_fasta_write, ">{}\n{}\n", i.header, i.upstream).expect("not able to write th line");
  }

  let mut downstream_fasta_write = File::create("selected-ids-downstream.fasta").expect("file not present");
  for i in final_struct.iter_mut(){
    write!(downstream_fasta_write, ">{}\n{}\n", i.header, i.downstream).expect("not able to write th line");
  }
